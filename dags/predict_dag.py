...
predict_output_path = f"predict_output/{todays_date}"
...
def train_dag():
    ...
    def featurize(data):
        ...
        save_features(features, target, predict_output_path)
        return

    @task()
    def train_model(arg):
        features, target = load_features(predict_output_path)
        model = get_power_forecaster()
        model.fit(X=features, y=target)
        predict = model.predict(features)

        with open(f"{predict_output_path}/model.pkl", 'wb') as file:
    ...
    def calculate_metrics(predict):
        target = pd.read_parquet(f"{predict_output_path}/target.parquet")
